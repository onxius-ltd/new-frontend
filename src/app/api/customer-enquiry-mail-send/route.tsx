...
// ─── Helper: section heading — uses border-left on a single td (email-safe) ──
      ...
      <table width="640" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="background:#e8521a;padding:28px 30px;text-align:center;">
            <img src="https://onxius.com/assets%2Flogo%2flight-logo.png" alt="Onxius" width="140"
        ...
        <tr>
          <td style="padding:10px 0 20px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
              ...
              ${row("Phone", `<a href="tel:${d.phone}" style="color:#e8521a;">${d.phone}</a>`)}
              ${row("Business Name", d["business-name"])}

              ${section("Project Type")}
              ${row("Service Needed", d.project, true)}
              ${row("New Build or Update?", d["service-type"])}
              ${d["service-type"] === "Yes — update existing" ? row("Existing Site URL", d["existing-app-web-link-update"]) : ""}
              ...
              ${d["domain-and-hosting"] === "Yes" ? row("Domain URL", d["existing-domain-link"]) : ""}
              ${row("Application Type", d["application-type"], true)}
              ${d["application-type"] === "Other" ? row("Other (described)", d["other-application-type"]) : ""}
              ...
              ${row("Professional Photos?", d["media-content"])}
              <tr>
                <td colspan="2" style="padding:10px 14px;border-bottom:1px solid #ede9e3;">
...
export async function POST(req: Request) {
  try {
    ...
    const { name, email, captchaToken } = data;

    // 1. Presence check
    if (!captchaToken) {
      return NextResponse.json({ success: false, message: "Captcha is required" }, { status: 400 });
    }
    if (!name || !email) {
